...
func TestFlowProfileDeletionRemainsBlockedWhileAFlowUsesTheProfile(t *testing.T) {
	...
	if _, err := r.Reconcile(ctx, request); err != nil {
		t.Fatalf("wait for schema readiness: %v", err)
	}
	waiting := &tamossv1alpha1.FlowProfile{}
	if err := c.Get(ctx, request.NamespacedName, waiting); err != nil {
		t.Fatalf("registered FlowProfile was removed before checking its database references: %v", err)
	}
	blocked := meta.FindStatusCondition(waiting.Status.Conditions, flowProfileConditionDeletionBlocked)
	if blocked == nil || blocked.Status != metav1.ConditionTrue || blocked.Reason != "WaitingForSchema" {
		t.Fatalf("expected deletion to wait for the schema: %#v", blocked)
	}
	if err := c.Create(ctx, schemaState); err != nil {
		t.Fatal(err)
	}
	if _, err := r.Reconcile(ctx, request); err != nil {
...
func TestFlowProfileDeletionWaitsForRegistrationBeforeCheckingReferences(t *testing.T) {
	ctx := context.Background()
	scheme := flowProfileTestScheme(t)
	profile := flowProfileFixture()
	profile.UID = "deleting-profile"
	profile.Finalizers = []string{flowProfileFinalizer}
	now := metav1.Now()
	...
	if err := controllerutil.SetControllerReference(profile, job, scheme); err != nil {
		t.Fatal(err)
	}
	schemaState := &corev1.ConfigMap{ObjectMeta: metav1.ObjectMeta{Name: tamoss.ResourceName("schema-state"), Namespace: tamoss.Namespace}, Data: map[string]string{schemaStateAppliedVersionKey: schemabundle.SchemaVersion}}
	c := fake.NewClientBuilder().WithInterceptorFuncs(fakeApplyInterceptor()).WithScheme(scheme).
	...
	if err := c.Get(ctx, client.ObjectKeyFromObject(job), job); err != nil {
		t.Fatalf("registration Job was removed before its result was known: %v", err)
	}
	job.Status.Conditions = []batchv1.JobCondition{{Type: batchv1.JobComplete, Status: corev1.ConditionTrue}}
	if err := c.Status().Update(ctx, job); err != nil {
		t.Fatal(err)
	...
	deletion := &batchv1.Job{}
	if err := c.Get(ctx, types.NamespacedName{Namespace: profile.Namespace, Name: flowProfileResourceName(profile, "delete")}, deletion); err != nil {
		t.Fatalf("successful registration without a state ConfigMap must still check Flow references: %v", err)
	}
}
