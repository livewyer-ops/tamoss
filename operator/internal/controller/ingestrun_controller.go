...
func (r *IngestRunReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if job != nil {
		...
		phase, reason, message, progressing := ingestPhaseFromJob(job, run.Status.ResultRef, r.currentTime(), stream)
...
func ingestPhaseFromJob(job *batchv1.Job, result tamossv1alpha1.IngestRunResultStatus, now time.Time, stream *ingestStreamSummary) (tamossv1alpha1.IngestRunPhase, string, string, bool) {
	if ingestJobFinished(job) {
		if stream == nil || !stream.RunFinished {
			return tamossv1alpha1.IngestRunPhaseRunning, "IngestResultPending", "The Job is terminal, but its TAMSin event stream is not yet available", false
		}
		if (stream.Outcome == ingestevent.RunSucceeded || stream.Outcome == ingestevent.RunPartial) &&
			...
			if ingestObservationDeadlineExpired(ingestJobTerminalReference(job), now) {
				return tamossv1alpha1.IngestRunPhaseFailed, "ResultVerificationTimeout", "A durable result was recorded but did not pass digest verification within the observation deadline", false
			}
			return tamossv1alpha1.IngestRunPhaseRunning, "ResultVerificationPending", "A durable result was recorded but has not passed digest verification", false
		}
		switch stream.Outcome {
		case ingestevent.RunSucceeded:
			return tamossv1alpha1.IngestRunPhaseSucceeded, "IngestSucceeded", "TAMSin completed the ingest run", false
		case ingestevent.RunPartial:
			return tamossv1alpha1.IngestRunPhasePartiallySucceeded, "IngestPartiallySucceeded", "TAMSin completed with both successful and failed inputs", false
		case ingestevent.RunInterrupted:
			return tamossv1alpha1.IngestRunPhaseFailed, "IngestInterrupted", "TAMSin was interrupted before the ingest completed", false
		case ingestevent.RunFailed:
			return tamossv1alpha1.IngestRunPhaseFailed, "IngestFailed", "TAMSin reported an ingest failure", false
		default:
			return tamossv1alpha1.IngestRunPhaseFailed, "IngestProtocolInvalid", "TAMSin reported an unknown terminal outcome", false
		}
	}
	if job.Status.Active > 0 {
		return tamossv1alpha1.IngestRunPhaseRunning, "IngestRunning", "TAMSin is processing media", true
	}
	if job.Status.StartTime != nil {
		return tamossv1alpha1.IngestRunPhaseRunning, "JobStarting", "The TAMSin Pod is starting", true
	}
	return tamossv1alpha1.IngestRunPhaseQueued, "JobQueued", "The TAMSin Job is waiting to be scheduled", true
}
...
func desiredIngestJob(run *tamossv1alpha1.IngestRun, spec tamossv1alpha1.IngestRunSpec, tamoss *tamossv1alpha1.Tamoss, endpoint, image, storageID, flowMetadata string, resolved ResolvedIngestInputs) *batchv1.Job {
	...
	return &batchv1.Job{
		...
		Spec: batchv1.JobSpec{
			// Retrying a whole ingest can repeat media writes; retry through a new IngestRun.
