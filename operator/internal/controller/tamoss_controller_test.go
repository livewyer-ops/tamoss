...
var _ = Describe("Tamoss Controller", func() {
	Context("When reconciling a resource", func() {
		...
		It("should block CNPG reconciliation when the dependency operator is missing", func() {
			...
			Expect(backendsReady.Message).To(ContainSubstring("CloudNativePG"))
			Expect(backendsReady.Message).To(ContainSubstring("postgresql.cnpg.io"))
			Expect(backendsReady.Message).To(ContainSubstring("select an external backend"))
			Expect(backendsReady.Message).NotTo(ContainSubstring("task "))
			degraded := meta.FindStatusCondition(updated.Status.Conditions, operatorstatus.ConditionDegraded)
			Expect(degraded).NotTo(BeNil())
			Expect(degraded.Status).To(Equal(metav1.ConditionTrue))
