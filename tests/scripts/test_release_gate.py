...
@pytest.mark.parametrize("version", ["8.2.0-oss1", "8.2.0-oss1-rc8", "8.2.0-oss2"])
def test_release_notes_require_a_reviewed_entry(tmp_path, version) -> None:
    ...
    result = subprocess.run(
        [sys.executable, str(REPO_ROOT / ".github/scripts/release-notes.py"), version],
        cwd=tmp_path,
        capture_output=True,
        text=True,
        check=False,
    )
    if version == "8.2.0-oss1":
        assert result.returncode == 0
        assert result.stdout == "Approved notes.\n\n### Added\n\n- Managed ingest.\n"
    else:
        assert result.returncode != 0
    ...
    release = next(
        step for step in steps if step["name"] == "Create draft GitHub Release"
    )
    assert release["with"]["body_path"] == "dist/operator-release/release-notes.md"
    assert not release["with"].get("generate_release_notes")
...
def test_ruff_hooks_use_the_ci_project_configuration() -> None:
    ...
    lock = tomllib.loads((REPO_ROOT / "src/uv.lock").read_text())
    version = next(p["version"] for p in lock["package"] if p["name"] == "ruff")
    assert config["repos"][0]["rev"] == f"v{version}"
