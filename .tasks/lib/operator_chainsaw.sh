...
certManager:
  enabled: true
cnpg:
  enabled: ${install_cnpg_operator}
rustfsOperator:
  enabled: ${install_rustfs_operator}
EOF
  ...
  if [ "$install_cnpg_operator" = "true" ]; then
    helmfile_args+=(--selector component=cnpg)
  fi
  if [ "$install_rustfs_operator" = "true" ]; then
    helmfile_args+=(--selector component=rustfs-operator)
  fi

  task_step "Operator Chainsaw: apply platform prerequisites" \
    task_platform_helmfile "$kubeconfig" "$platform_helmfile" "$platform_values" 5m "${helmfile_args[@]}"
