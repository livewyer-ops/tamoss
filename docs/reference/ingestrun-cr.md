...
| `.status.output.memberFlowsTruncated` | True when the validated event stream contained more member Flows than fit in bounded Kubernetes status. Inspect the root Flow's collection through TAMS for the complete graph. |
