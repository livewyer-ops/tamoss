...
`Tamoss` YAML fields directly in
`instances/tamoss-single-server/tamoss.yaml` when you need different
...
Raise the resources of the single pods in
`instances/tamoss-single-server/tamoss.yaml` when the
...
command. Retrieve credentials with
`task env:credentials ENV=my-single-server INSTANCE=tamoss-single-server KUBECONFIG="$KUBECONFIG"`.
Supply browser login credentials through the target's `TEST_TAMOSS_AUTH_USER`
and `TEST_TAMOSS_AUTH_PASSWORD` settings or its password Secret reference.
