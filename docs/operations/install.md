...
shared prerequisites through `deploy/platform/chart` and the environment's
`platform-values.yaml`. The operator layer installs the TAMOSS CRDs, controller,
RBAC, and webhooks. The environment layer applies one or more namespaced `Tamoss`
custom resources.
