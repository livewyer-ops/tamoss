...
describe("operational routes", () => {
  ...
  it("uses the TAMOSS artwork in navigation", async () => {
    renderRoute("/");
  ...
  it.each(["folder/clip", "folder%2Fclip", "clip%20 café", " clip "])(
    "preserves Object identity in the route for %s",
    async (objectId) => {
      renderRoute(`/objects/${encodeURIComponent(objectId)}`);
  ...
  it.each([".", ".."])("rejects %s in Object lookup", async (objectId) => {
    renderRoute("/objects");
    const input = await screen.findByRole("textbox", { name: "Object ID" });
    fireEvent.change(input, { target: { value: objectId } });
    expect(input).toBeInvalid();
    fireEvent.click(screen.getByRole("button", { name: "Inspect" }));
    expect(window.location.pathname).toBe("/objects");
    expect(mocks.api.getObject).not.toHaveBeenCalled();
  });
