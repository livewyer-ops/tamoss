...
TAMS reads use `TamossApiClient`, which inherits the shared `ApiTransport`
request path. Console reads and commands use the adapters under `src/control`.
Endpoint response aliases in
`src/types/tams.ts` are derived from the generated contract and keep local
typing choices close to the API boundary.

Detail pages call TanStack Query's `useQuery` with the client from `useApi()`
...
Catalogue routes page through `useCursorPage` (`src/hooks/useCursorPage.ts`),
which holds cursor state and request cancellation.
