...
function abortError(): DOMException {
  return new DOMException(
    "The synchronised audio track was removed.",
    "AbortError",
  );
}
...
function safePause(media: HTMLMediaElement): void {
  try {
    ...
    // A detached or not-yet-initialised media element is already stopped.
  }
}
...
export function createSynchronizedAudioSidecar(
  ...
  const timeout = window.setTimeout(() => {
    fail(
      new AudioSidecarError(
        "timeout",
        "The synchronised audio track did not become ready in time.",
      ),
    );
  ...
  if (!Number.isFinite(offsetSeconds)) {
    fail(
      new AudioSidecarError(
        "load-failed",
        "The synchronised audio track could not be loaded.",
      ),
    );
    ...
    fail(
      new AudioSidecarError(
        "unsupported",
        "Synchronised audio playback is not supported by this browser.",
      ),
    );
  } else {
    try {
